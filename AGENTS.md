...
Next.js 16 + React 19 + TypeScript + Tailwind v4, npm.
...
No unit test runner is configured yet, so tests are not a build gate. Run
`/tests` or `$tests` to add one and update this section with the real test
commands.

No Verify command or automatic GitHub checks exist yet. Run `/ci` or `$ci` to
set one up.
