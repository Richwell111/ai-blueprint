...
> Your conventions, tuned by `/onboard` for this project's real stack:
> Next.js 16 (App Router) + React 19 + TypeScript + Tailwind v4. No database,
> ORM, or auth provider is chosen yet (`blueprint/project-plan.md` section 5 is
> still blank) - the Database and Data Fetching sections below are placeholders
> to fill in once that's decided, likely during `/overview` or the first
...
No `src/` directory; `app/` sits at the project root.
...
- shadcn/ui is not installed yet; add it (`components.json`) if the project
  adopts it, otherwise keep components hand-rolled with Tailwind
- No inline styles
- `app/globals.css` currently follows the system color scheme
  (`prefers-color-scheme`); revisit if the project wants dark mode first

## Database

> TODO: no database or ORM is chosen yet. Fill this in once `project-plan.md`
> section 5 (Tech) is answered and `/overview` has run.

## Data Fetching

- Server components fetch data directly where possible
- Client components use Server Actions for mutations
- Validate all inputs with Zod once external input is accepted
> TODO: no auth provider is chosen yet. Once one is picked, scope every
> user-owned query by the authenticated user id, never a client-supplied one.
